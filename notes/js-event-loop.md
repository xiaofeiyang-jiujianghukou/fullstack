...
`await` 一定会「切一刀」，但**切下来的后半段什么时候进微任务队列，取决于被 await 的对象是不是已完成态**。这是最容易漏掉的细节。

| 被 await 的东西 | 后半段何时入微任务队列 |
|---|---|
| 已完成态（`null`、`123`、`Promise.resolve()`、已 settled 的 Promise） | **当场**——同步阶段就排好了 |
| pending（`fetch`、真 I/O、`new Promise(r => setTimeout(r))`） | 挂在那个 Promise 上，**等它 resolve 才入队** |

同一段代码，只换 `await` 的对象，输出顺序就变：

```js
rec('A')
setTimeout(() => rec('B'), 0)
process.nextTick(() => rec('C'))
async function task() { rec('D'); await X; rec('E') }
task()
...
```
X = null（已完成）  →  A → D → G → C → E → F → B
X = 真异步操作      →  A → D → G → C → F → B → E
```

- **`X = null`**：`null` 被包成 `Promise.resolve(null)`，已完成 → 同步阶段结束时微任务队列已是 `[E, F]`（`task()` 的调用早于 `Promise.resolve()`，E 先登记），所以 E 抢在 F 前面
- **`X = 真异步`**：同步阶段结束时队列只有 `[F]`，E 还没影，得等 X 完成——落到宏任务 B 之后

**看到 `await X`，先问一句「X 现在就是完成态吗」。** 是 → 后半段当场入队；否 → 等 X resolve 才入队。

`await null` 是这条规则的极端案例：等的压根不是异步操作，但「切一刀 + 当场入队」照做不误。**`await` 的语义不是「等异步完成」，而是「到此切一刀」。**
...
**变体**：把 `await null` 换成 `await fetch(...)` 这类真异步，答案变成 **A → D → G → C → F → B → E**——E 得等 Promise 真 resolve 才入队，只能排到宏任务 B 后面。原理见第 5 节「await 之后的代码，何时进队列」。
...
| 「`await` 之后的代码是 async 的收尾，慢一档，最后才轮到」 | 它就是**普通微任务**，与 `.then` 同级同队，**必然先于任何宏任务** |
| 「`await` 之后的代码要等被 await 的东西完成才入队」 | 只有 pending 才这样；`await null` 这类已完成态**当场入队**（见第 5 节） |

前三条的根因是同一个：**用「执行流」思维看代码（这行完了下一行），而 JS 需要「队列调度」思维。**

后两条的根因是**归类不清**——调度规则本身都懂，但没把「`await` 之后的代码」归到微任务那一类，规则就应用不上。判断顺序时，先给每段代码贴好标签（同步 / nextTick / 微任务 / 宏任务），再套规则。
